...
module Xt : sig
  (** Explicit transaction log passing on shared memory locations.
      ...
      that the side-effects are also performed multiple times.
      {!Xt.post_commit} can be used to perform an action only once after the
      transaction has been committed succesfully.
      ...
      within transactions.  There are advanced algorithms where one might,
      within a transaction, perform operations that do not get recorded into the
      transaction log.  Using such techniques correctly requires expert
      knowledge and is not recommended for casual users.

      As an example, consider an implementation of doubly-linked circular
      lists. Instead of using a mutable field, [ref], or [Atomic.t], one would
      use a shared memory location, or {!Loc.t}, for the pointers in the node
      type:
      ...
      that removal becomes an {{:https://en.wikipedia.org/wiki/Idempotence}
      idempotent} operation.  Using explicit transaction log passing one could
      implement the [remove] operation as follows:
